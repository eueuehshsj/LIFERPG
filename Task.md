...
1. 포인트를 1단위로 조절
2. 완료 버튼을 한 번 눌렀을때 여러 퀘스트 완료 가능
3. 다시뽑기를 누르면 즉시 룰렛 작동
4. 퀘스트 삭제 확인창 추가
5. 일정이 화면을 넘어가면 스크롤 생성, 화면비율은 유지
6. 퀘스트, 보상 작성, 수정시 글자 제한 추가: 제목 50, 내용 500, 이름 20
7. 퀘스트, 보상 내용이 숫자와 영어 일 경우 줄바뀜이 작동하지 않는 것 수정
8. 게시판에 부착된 퀘스트 종이가 내용에 맞게 수정
9. 보상 수령의 랜덤뽑기 보상 룰렛 화면 크기 고정

---

## 최적화 수정사항 1

1. App.tsx를 기능별 컴포넌트로 나눠서 분리

---

## 최적화 수정사항 2

1. 인라인 스타일을 상수나 클래스 기반으로 정리
...
6. [x] 연속 완료(스트릭) / 레벨 시스템 — RPG 테마에 맞는 게이미피케이션 요소 추가
   - 퀘스트 완료 시 오늘 날짜를 `completedTasks`와는 독립된 날짜 로그(`completedDates`)에 기록해, 완료 목록 삭제로 스트릭이 깨지지 않도록 함. 오늘/어제 중 하루라도 완료 기록이 있으면 스트릭 유지(그레이스 기간), 이틀 이상 공백이면 0으로 초기화.
   - 누적 획득 포인트 100pt당 레벨이 오르는 시스템을 추가하고, 목판 헤더 아래에 스트릭/레벨 뱃지를 표시. 레벨업 시 골드 글로우 애니메이션으로 시각 피드백 제공.
...
9. [x] 퀘스트 정렬/필터 — 마감일순 정렬, 우선순위 필터 등
   - 게시판 상단에 우선순위 필터 칩(전체/보통/중요/긴급)과 마감일순 정렬 토글을 추가 (`TaskBoard` 로컬 상태로 관리, 마감일 없는 퀘스트는 정렬 시 뒤로 배치).
   - 필터 결과가 없을 때와 퀘스트 자체가 없을 때의 안내 문구를 구분.
10. [x] 접근성 개선 — 아이콘 기반 커스텀 버튼에 `aria-label` 추가
   - 각 모달의 닫기(X) 버튼과 `RemoveRewardModal`의 개별 보상 삭제(Trash2) 버튼에 `aria-label`을 추가.
   - 하단에 텍스트 "닫기" 버튼이 별도로 있는 모달(`DataBackupModal`, `RemoveRewardModal`, `CompletedTasksModal`)은 접근성 이름 중복을 피하기 위해 모달 맥락을 포함한 라벨을 사용.
